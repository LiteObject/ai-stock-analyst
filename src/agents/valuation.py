"""Valuation analysis agent using DCF and Owner Earnings methods."""
...
def valuation_agent(state: AgentState):
    ...
    - Buffett's Owner Earnings method

    Args:
        state: Current agent state with ticker and date info

    Returns:
        Updated state with valuation analysis signals
    """
    data = state["data"]
    ticker = data["ticker"]
    ...
    if earnings_growth is None:
        # Use a conservative default growth rate
        earnings_growth = 0.05  # 5% default growth rate
        logger.warning(f"No earnings growth available for {ticker}, using default 5%")
